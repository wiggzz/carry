...
class BenchmarkEc2BootstrapContracts(unittest.TestCase):
    ...
    def test_workflow_launches_only_canonical_workers_and_always_cleans_its_instance(self):
        ...
        worker_tag_line = next(line for line in workflow.splitlines() if line.lstrip().startswith('TAGS='))
        self.assertIn("ManagedBy,Value=carry-swebench", worker_tag_line)
        self.assertIn("RunId,Value=$RUN_ID", worker_tag_line)
